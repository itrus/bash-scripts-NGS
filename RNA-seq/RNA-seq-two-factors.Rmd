...
proceedToGO <- TRUE
FDRLimit <- 0.25
GMTFileName <- data.frame(BP = "csabaeus.GO_BP.name.gmt",
                          CC = "csabaeus.GO_CC.name.gmt",
                          MF = "csabaeus.GO_MF.name.gmt",
                          HP = "csabaeus.HP.name.gmt")
...
par(mfrow = c(1, 1))
nice30colors <- c('black','forestgreen','red2','orange','cornflowerblue',
...
for(i in 1:length(factorsList)){
  ...
  if (nlevels(col.group)==2){
      ...
      } else levels(col.group) <- c(brewer.pal(12, "Paired"),
                                    nice30colors, nice30colors[1:3])
...
geneExpression$samples$lev <- paste(geneExpression$samples$cell,
                                    geneExpression$samples$infection,
                                    geneExpression$samples$time,
                                    sep=".")
geneExpression$samples$lev
f <- factor(geneExpression$samples$lev)
f
modelDesign <- model.matrix(~0+f, data = geneExpression$samples)
#colnames(ModelDesign) <- unique(GeneExpression$samples$lev)
modelDesign
expressionValues <- voom(geneExpression, modelDesign, plot = T)
modelFit <- lmFit(expressionValues, modelDesign)

cont.dif <- makeContrasts(
  Difference = (fWT.zika.24h-fWT.mock.24h)-
    (fKO.zika.24h-fKO.mock.24h),
  levels = modelDesign
)

cont.dif

modelFit2 <- contrasts.fit(modelFit, cont.dif)
modelFit2 <- eBayes(modelFit2)
result <- topTable(modelFit2, adjust = "BH", number = Inf)
...
for(i in 1:1){
  plotMD(modelFit2, status = rownames(expressionValues$genes) %in% result$GeneId, coef = i)
  abline(h = c(-1, 1), col = "blue")
  volcanoplot(modelFit2, coef = i, names =
                geneExpression$genes$external_gene_name,
...
runGO <- function(GMTFileName, shortCutName){
  ...
  print(modelDesign)
  print(cont.dif)
  result <- camera(expressionValues, countedGene, modelDesign,
                   contrast = cont.dif, sort = T)
