...
proceedToGO <- FALSE
FDRLimit <- 0.25
GMTFileName <- data.frame(BP = "sscrofa.GO_BP.name.gmt",
                          CC = "sscrofa.GO_CC.name.gmt",
                          MF = "sscrofa.GO_MF.name.gmt",
                          HP = "sscrofa.HP.name.gmt",
                          REAC = "sscrofa.REAC.name.gmt")
geneSetOfInterest <- "antiviral innate immune response	"
```
...
librarian::shelf(magrittr, DT, ggplot2, xfun, dplyr, janitor,
                 ...
                 pheatmap, stringr, limma, GSEABase,
                 quiet = T)
...
String "**Sscrofa11.1**" corresponds to the current version of BioMart for **porcine genome**. Searched BioMart: **`r currentBioMart`** The current block creates database of Genes and Transcripts called *`r filename`*. It will be created once and re-loaded from the saved file if re-ruinning the script.
...
kallistoData <- tximport(filenamesKallisto, type = "kallisto",
                         tx2gene = dplyr::rename(Tx2Gen[, c(1, 3)],
...
for(i in 1:length(factorsList)){
  ...
  if (nlevels(col.group) == 2){
      ...
      plotMDS(lcpm, labels = group, col = col.group)
        ...
        plotMDS(lcpm, labels = group, col = brewer.pal(nlevels(col.group), "Paired"))
        }
...
for(i in 1:length(factorsList)){
  ...
  if (nlevels(col.group) == 2){
      ...
      plot(PCA$x[, 1:2], cex = 2, pch = 20, col = col.group)
        ...
        plot(PCA$x[, 1:2], cex = 2, pch = 20,
             col = brewer.pal(nlevels(col.group), "Paired"))
        }
  abline(h = 0, v = 0)
...
```{r GO}
collapse_gmt_duplicates <- function(infile, outfile) {
  lines <- readLines(infile, warn = FALSE)
  parts <- strsplit(lines, "\t", fixed = TRUE)

  go_id <- vapply(parts, `[`, character(1), 1)
  desc  <- vapply(parts, function(x) if (length(x) >= 2) x[2] else NA_character_, character(1))
  genes <- lapply(parts, function(x) if (length(x) >= 3) x[3:length(x)] else character(0))

  # union genes per GO ID
  gene_union <- tapply(seq_along(genes), go_id, function(ix) unique(unlist(genes[ix], use.names = FALSE)))
  # keep first description per GO ID (or choose most frequent if you prefer)
  desc_first <- tapply(desc, go_id, function(x) x[which(!is.na(x))[1]])

  out_lines <- mapply(function(id, d, g) {
    paste(c(id, d, g), collapse = "\t")
  }, names(gene_union), desc_first[names(gene_union)], gene_union, USE.NAMES = FALSE)

  writeLines(out_lines, outfile, useBytes = TRUE)
  outfile
}

runGO <- function(CurrentGMTFileName, shortCutName){
  
  gmt_clean <- collapse_gmt_duplicates(CurrentGMTFileName, tempfile(fileext = ".gmt"))
  gmt_s4 <- GSEABase::getGmt(gmt_clean)
  geneIndex <- GSEABase::geneIds(gmt_s4)
  
  go_lookup <- data.frame(
    GO.ID = names(geneIndex),
    Description = sapply(gmt_s4, function(x) x@shortDescription),
    stringsAsFactors = FALSE
  )
  
  # Counting genes
  countedGene <- ids2indices(
    geneIndex,
    id = expressionValues$genes$external_gene_name
  )
  
  # Model (unchanged)
  groupFactor <- as.factor(expressionValues$targets$group)
  ...
  result <- camera(
    ...
    countedGene,
    GOModelDesign,
    contrast = GOcontrastsMatrix,
    sort = TRUE
  )

  result$GO.ID <- rownames(result)

  result <- merge(result, go_lookup,
                by = "GO.ID",
                all.x = TRUE,
                sort = FALSE)

  result <- result[, c("GO.ID", "Description",
                     "NGenes", "Direction",
                     "PValue", "FDR")]
  ...
  # saving GO results
  write.csv(result, file = paste("GO", shortCutName,
                                 gsub(":", "-", Sys.time()),".csv"))
  ...
  #                                    geneListGProfiler)
  # write_delim(GProfilerFormatResult, paste("GO for Cytoscape", shortCutName,
  #                   gsub(":", "-", Sys.time()),".txt"), delim = "\t")
  return(result)
}

for(i in 1:dim(GMTFileName)[2]){
  print(paste("Running GO analysis for GMT file:", colnames(GMTFileName[i])))
  result <- runGO(GMTFileName[ , i], colnames(GMTFileName[i]))
