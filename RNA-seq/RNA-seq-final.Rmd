...
# Importing Data and selecting samples for the run
...
GeneExpression <- GeneExpression[, GeneExpression$samples$timepoint == "0h"]
#GeneExpression <- GeneExpression[, GeneExpression$samples$cell == "WT"]
#GeneExpression <- GeneExpression[, GeneExpression$samples$lib.size > 100000]

# setting up main parameters
FactorsList <- colnames(GeneExpression$samples)[-2:-3]
...
write.csv(GeneExpression, file = paste("GE normalized", gsub(":", "-", Sys.time()),".csv"))
```
...
ModelDesign <- model.matrix(~cell, data = GeneExpression$samples)
...
For volcano plots Top10 genes are named. Grey dashed line: threshold for non-adjusted p-value of 0.05. Blue and red lines: thresholds for -1 and +1 FC.")
...
## Analysis
GroupFactor <- ExpressionValues$targets[FactorsList[CurrentFactor]][, 1] %>% factor
...
Result <- camera(ExpressionValues, CountedGene, Design, contrast = ContrastsMatrix,
                 sort = T)
...
# saving GO results
write.csv(as.data.frame(Result), file = paste("GO", gsub(":", "-", Sys.time()),".csv"))
...
write_delim(GProfilerFormatResult, paste("GO for Cytoscape", gsub(":", "-", Sys.time()),".txt"), delim = "\t")
```
